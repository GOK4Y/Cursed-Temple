...
def draw_intro_scene():
    ...
    glutSolidTorus(0.1, 0.7, 12, 36)
    glPopMatrix()
    dx = player_pos[0]
    dz = player_pos[1] + 1.5
    if dx*dx + dz*dz < 0.7**2:
        change_scene("temple")
    glutSwapBuffers()
...
def keyboard(key, x, y):
    ...
    # Bu kontrol kalktı: sadece temple sahnesinde hareket etme kısıtlaması
    # Onun yerine sahne farkına göre BOUNDS sınırları değiştirilecek
    step = PLAYER_SPEED
    front = get_front_vector()
    ...
    # Sahneye göre sınırlandır
    if scene_state == "temple":
        limit = BOUNDS - 0.5
    else:
        limit = 2  # intro odası küçük

    player_pos[0] = max(-limit, min(limit, player_pos[0]))
    player_pos[1] = max(-limit, min(limit, player_pos[1]))

    check_collisions()  # temple dışında kontrol edilmez ama zararı yok
